...
const MockTestscet = () => {
  ...
  const mockTests = Array.from({ length: 30 }, (_, i) => ({
    ...
    id: i + 1,
    isLive: i < 3  // Only first 3 tests are live
  }));

  const handleStartTest = (testName, testIdcet) => {
    setSelectedTest({ name: testName, id: testIdcet });
    setCountdown(10);
    ...
    const countdownInterval = setInterval(() => {
      setCountdown(prevCountdown => {
        if (prevCountdown <= 1) {
          ...
          navigate(`/instructionscet/${testIdcet}`);
          return 0;
        }
